...
impl House {
    ...
    pub fn add_device<T: SmartDevice + 'static>(&mut self, room_id: &str, device: T) -> Result<(), Error> {
    ...
    pub fn visit_devices_mut<T: DeviceVisitor>(&mut self, visitor: &mut T, room_id: Option<&str>) -> Result<(), Error> {
        match room_id {
            Some(name) => {
    ...
    pub fn visit_devices<T: DeviceVisitor>(&self, visitor: &mut T, room_id: Option<&str>) -> Result<(), Error>{
        match room_id {
            Some(name) => {
    ...
    pub fn extract_device<T: 'static>(&mut self, room_id: &str, device_id: &str) -> Result<T, Error> {
        match self.rooms.get_mut(room_id) {
            Some(room) => {
                room.extract_device(device_id)
            },
