...
pub struct Room {
    name: String,
    pub(crate) devices: HashMap<String, Box<dyn Any + Send + Sync>>, // device_id->device
}
...
impl Room {
    ...
    pub(crate) fn add_device<T: SmartDevice + 'static>(
        &mut self,
        device: T,
    ) -> Result<(), HouseError> {
    ...
    pub(crate) fn extract_device<T: 'static>(&mut self, device_id: &str) -> Result<T, HouseError> {
            ...
            Some(any_device) => {
                match any_device.downcast::<T>() {
                    ...
                    Err(any_device_back) => {
                        ...
                        Err(HouseError::WrongDeviceType(device_id.to_string()))
                    }
                }
            }
            None => Err(HouseError::DeviceNotFound(device_id.to_string())),
        }
    }
